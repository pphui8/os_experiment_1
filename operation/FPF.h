...
class FPF;
bool
    comp2(const process &a, const process &b);

class FPF {
public:
    FPF(process pro[100], int n): length(n), time(1), AWTT(0) {
...
FPF::add_process()
{
    ...
    //加入一个就排序
    sort(wait_queue.begin(), wait_queue.end(), comp2);
}

#include<iostream>
using namespace std;
inline void
FPF::print_data(const process &pro)
{
    if(pro.pcb.state == 1) {
